...
#define ROMCFE "/rom/cfe/cfe_rt-ac3100.bin"
...
void update_cfe_k3(void)
{
	...
	if (!check_if_file_exist(ROMCFE) || cfe_nvram_match("bl_version", "3.0.0.3"))
		return;
	//if (pids("envrams"))
	//sleep(5);
	...
	*/
	...
	//logmessage("K3INIT", "CFE has upgraded already!");
...
void k3_init_done(void)
{
    ...
    system("/usr/bin/khttpd.sh &");
if (!nvram_match("sc_mode", "0"))
{
